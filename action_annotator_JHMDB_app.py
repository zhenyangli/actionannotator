...
def make_video_dict():
    ...
    #videos = sorted(glob('static/JHMDB/videos_annotator/*'))
    #random.seed(1234)
    #random.shuffle(videos)
    
    #fp = open('JHMDB_videos.txt','w')
    #for video in videos:
    #    fp.write(video+'\n')
    #fp.close()

    with open('JHMDB_videos.txt', 'r') as fp:
        videos = fp.read().splitlines()

    for i, video in enumerate(videos):
        video_number = str(i)
        video_name = video.split('/')[-1][:-4]
        video_file = ''.join(['/static/JHMDB/videos_annotator/', video_name, '.mp4'])
        text_file = ''.join(['static/JHMDBEntities/', video_name,'.xml'])
        d[video_number] = (video_name, video_file, text_file)
    return d
...
def annotation_page(video_number):
    ...
    if video_number in video_dict:
        video_name, video, comments = video_dict[video_number]
        ...
        #
        return render_template('annotation_JHMDB_page.html',
                                video_name = video_name[:maxlen],
...
def annotator_page(video_number):
    ...
    if video_number in video_dict:
        ...
        #
        return render_template('annotator_JHMDB_page.html',
                                video_name = video_name[:maxlen],
                                video_number = video_number,
                                video = video,
                                current = current,
